...
def run_benchmarking(
    ...
    if precomputed_feature_maps is None:
        ...
        image_path, image_transforms = (
            benchmark.stimulus_data.image_path,
            model_option["transforms"],
        )
        stimulus_loader = DataLoader(
            StimulusSet(image_path, image_transforms), batch_size=64
        )
        feature_maps = get_all_feature_maps(
            model_option["model"], stimulus_loader, layers_to_retain
        )
        feature_maps_redux = srp_extraction(
            model_string,
    ...
    if precompute_rdms:
        feature_map_rdms = {
            model_layer: {
                "train": 1 - np.corrcoef(feature_maps[model_layer][::2, :]),
    ...
    for model_layer_index, model_layer in enumerate(
        tqdm(feature_maps_redux, "Mapping (Layer)")
    ):
        ...
        best_alpha_idx = np.array(
            [alpha_values.index(alpha_) for alpha_ in regression.alpha_]
        )
        predictions = {
            "train": np.take_along_axis(
                regression.cv_results_, best_alpha_idx[None, :, None], axis=2
            )[:, :, 0],
            "test": xy["test"]["X"].dot(regression.coef_.transpose())
            ...
            if metric == "wrsa":
                for score_set in ["train", "test"]:
                    for region in benchmark.rdms:
                        for subj_id in benchmark.rdms[region]:
                            response_indices = rdm_indices[region][subj_id]
                            prediction_subset = predictions[score_set][
                                :, response_indices
                            ]
...
def get_results_max(results, average_over=None, average_when="after"):
    ...
    if average_over and average_when == "before":
        group_vars = [
            var for var in results.columns if var not in average_over + ["score"]
        ]
    ...
    results = (
        results.copy()
        .pivot_table(index=index_cols, values="score", columns="score_set")
        .reset_index()
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_string", required=True, type=str, help="name of deep net model to load"
    )
    parser.add_argument(
        "--benchmark", required=True, type=str, help="name of benchmark data to assess"
    )
    parser.add_argument(
        "--randomize",
        required=False,
    ...
    parser.add_argument(
        ...
        type=str,
        default="parquet",
        help="format to use when saving results",
    )
    parser.add_argument(
        "--layer_splits",
        required=False,
        type=int,
        default=0,
        help="number of times to split the layers",
    )
    parser.add_argument(
        "--cuda_device",
        required=False,
        type=str,
        default="",
        help="cuda device to use for extraction",
    )
    ...
    for output in outputs:
        ...
        output_files[output] = os.path.join(
            output_dir, model_string.replace("/", "-") + "." + output_ext
        )
    ...
    if not all([os.path.exists(output_file) for output_file in output_files.values()]):
        ...
        model_layer_names = get_empty_feature_maps(
            model_option["model"], names_only=True, input_shape=(3, 224, 224)
        )
