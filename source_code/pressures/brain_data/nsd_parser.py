...
def _nsd_path_prompt():
    ...
    while True:
        ...
        if os.path.exists(user_input):
            ...
            return user_input
        print(
            "Invalid path. Please try again or type 'cancel' to exit.", file=sys.stderr
        )
...
def load_voxel_info(subj, space):
    ...
    if space == "func1pt8mm":
        ...
        roifiles = np.sort(
            [fn for fn in listdir(roidir) if not "lh" in fn and not "rh" in fn]
        )
        ...
        for fn in np.sort(
            [
                fn
                for fn in listdir(f"{SURFDIR}/{subj}/label")
                if fn[-4:] == ".mgz" or ".ctab" in fn
            ]
        ):
            if ".mgz.ctab" in fn:
                annot_fns.append(fn)
        ...
        roidata["ncsnr"] = (
            nib.load(f"{BETADIR}/{subj}/{space}/{BETA_VERSION}/ncsnr.nii.gz")
            .get_fdata()
            .reshape(-1)
        )
        ...
        for domain in ["faces", "bodies", "word", "places"]:
            tvals = (
                nib.load(
                    f"{NSD_PATH}/nsddata/ppdata/{subj}/{space}/floc_{domain}tval.nii.gz"
                )
                .get_fdata()
                .reshape(-1)
            )
        ...
        for roifile in roifiles:
            ...
            if roifile.split(".")[0] + ".mgz.ctab" in annot_fns:
                ...
                annots = (
                    pd.read_csv(
                        join(
                            f"{SURFDIR}/{subj}/label",
                            roifile.split(".")[0] + ".mgz.ctab",
                        )
                    )
                    .iloc[:, 0]
                    .values
        ...
        # get list of files that have roi info
        roifiles = np.sort(
            [fn for fn in listdir(roidir) if fn[-4:] == ".mgz" or ".ctab" in fn]
        )
        ...
        for hemi in hemis:
            ...
            roidata[f"{hemi}.ncsnr"] = np.squeeze(
                np.array(
                    nib.load(
                        f"{BETADIR}/{subj}/{space}/{BETA_VERSION}/{hemi}.ncsnr.mgh"
                    ).get_fdata()
                ).T
            )
            ...
            for roifile in roifiles:
                ...
                if hemi in roifile:
                    X = np.squeeze(
                        np.array(nib.load(join(roidir, roifile)).get_fdata()).T
                    )
                    ...
                    if roifile[3:] + ".ctab" in annot_fns:
                        ...
                        # parse
                        annots = (
                            pd.read_csv(join(roidir, roifile[3:] + ".ctab"))
                            .iloc[:, 0]
                            .values
                        )
                        annots_dict = dict()
                        ...
                        roidata[".".join(roifile.split(".")[:2]) + ".label"] = np.array(
                            X_annots
                        )
...
def load_NSD_voxel_metadata(
    subjs, roi_group, space, voxels_to_include=None, savedir=None, overwrite=False
):
    ...
    for subj in subjs:
        ...
        if savefile_exists and not overwrite:
            ...
            if voxels_to_include is None:
                ...
                for h, hemi in enumerate(hemis):
                    ...
                    if roi_group == "categ-selective":
                        ...
                        include = np.isin(
                            roi_dfs[h][f"{htag}HCP_MMP1.label"].values.astype(str),
                            VTC_rois,
                        )
            ...
            if space == "func1pt8mm":
                ...
                indices = np.array(
                    [f"({i})" for i in np.arange(len(all_included_voxels))]
                )
            ...
            if space == "func1pt8mm":
                ...
                voxel_meta_df = pd.concat(voxel_meta_df, sort=True).iloc[
                    all_included_voxels
                ]
            ...
            if savedir:
                np.save(savefile, voxel_metadata[subj], allow_pickle=True)
...
def load_NSD_brain_data(
    subjs, space, roi_group, voxel_metadata, annotations, savedir, output=False
):
        ...
        if not exists(join(savedir, f"{subj}_{space}_{roi_group}_brain_data_df.pkl")):
            ...
            for ses in tqdm(range(1, this_subsess + 1)):
                ...
                if space == "func1pt8mm":
                    betafns = [
                        join(
                            ...
                            f"betas_session{ses:02d}.nii.gz",
                        )
                    ]
                elif space == "nativesurface":
                    betafns = [
                        join(
                            ...
                            space,
                            "betas_fithrf_GLMdenoise_RR",
                            f"{hemi}.betas_session{ses:02d}.hdf5",
                        )
                        for hemi in ["lh", "rh"]
                    ]
                elif space == "fsaverage":
                    betafns = [
                        join(
                            ...
                            space,
                            "betas_fithrf_GLMdenoise_RR",
                            f"{hemi}.betas_session{ses:02d}.mgh",
                        )
                        for hemi in ["lh", "rh"]
                    ]
                ...
                for betafn in betafns:
                    ...
                    if space == "func1pt8mm":
                        ...
                        for h_ in hf:
                            ...
                            hemi_included_voxels = voxel_metadata[roi_group][subj][
                                "hemi_included_voxels"
                            ]
                            incl_idx = np.squeeze(
                                np.argwhere(hemi_included_voxels[x])
                            ).astype(int)

                            betas.append(
                                stats.zscore(
                                    np.array(h_["betas"])[:, incl_idx], axis=zaxis
                                )
                            )
                            x += 1
                        ...
                        betas = [
                            np.squeeze(np.array(nib.load(betafn).get_data()).T)
                            for betafn in betafns
                        ]
                ...
                if space == "func1pt8mm":
                    ...
                    included_voxels = voxel_metadata[roi_group][subj][
                        "all_included_voxels"
                    ]
            ...
            brain_data_df.to_pickle(
                join(savedir, f"{subj}_{space}_{roi_group}_brain_data_df.pkl")
            )
            ...
            if output is True:
                brain_data[subj] = pd.read_pickle(
                    join(savedir, f"{subj}_{space}_{roi_group}_brain_data_df.pkl")
                )
...
def load_NSD_benchmark_ROI_metadata(
    subjs,
    space,
    ncsnr_threshold=0.2,
    t_threshold=1,
    ...
    **kwargs,
    ...
    def get_idx_by_roi_group(
        metadata, group_label, roi_names, logic="include", t_threshold=None
    ):
        ...
        if space == "func1pt8mm":
            ...
            condition = [
                logic(metadata[f"{h}.{group_label}.label"].values, roi_names)
                for h in ["lh", "rh"]
            ]
        ...
        if t_threshold:
            if space == "func1pt8mm":
                lowt_idx = np.squeeze(
                    np.argwhere(metadata[f"{group_label}.tval"].values < t_threshold)
                )
            else:
                lowt_idx = np.squeeze(
                    np.argwhere(
    ...
    for roi_group in roi_group_iterator:
        ...
        roi_group_iterator.set_description(
            f"Loading voxel metadata for ROI group ({roi_group})"
        )

        all_metadata[roi_group] = load_NSD_voxel_metadata(
            subjs, roi_group, space, voxels_to_include, savedir, overwrite
        )
    ...
    for subj in subj_iterator:
        ...
        for i, roi_group in enumerate(roi_groups):
            ...
            this_included_voxels = deepcopy(
                all_metadata[roi_group][subj]["all_included_voxels"]
            )
            ...
            if "nsdgeneral" in roi_group or "early-visual" in roi_group:
                ...
                if "streams" in roi_group:
                    ...
                    group_label, roi_names = "streams", [
                        "early",
                        "midparietal",
                        "parietal",
                    ]
                    excl_idx = get_idx_by_roi_group(
                        this_metadata, group_label, roi_names, "exclude"
                    )
                    ...
                    excl_idx = get_idx_by_roi_group(
                        this_metadata, group_label, roi_names, "include", t_threshold
                    )
                    ...
                    group_label, roi_names = "prf-visualrois", [
                        "V1v",
                        "V1d",
                        "V2v",
                        "V2d",
                ...
                if ncsnr_threshold:
                    if space == "func1pt8mm":
                        lowrel_idx = np.squeeze(
                            np.argwhere(this_metadata["ncsnr"].values < ncsnr_threshold)
                        )
                    else:
                        or_filter = np.logical_or(
                            this_metadata["lh.ncsnr"].values < ncsnr_threshold,
        ...
        if len(roi_groups) >= 2:
            ...
            roi_incl_idx = np.logical_and(
                all_incl_idx[0], all_incl_idx[1]
            )  # all IDX in nsdgeneral + streams
            roi_incl_idx = np.logical_or(
                roi_incl_idx, np.sum(np.vstack(all_incl_idx[2:]), axis=0) > 0
            )  # or ROI
